...
export class MapEngineTestHelper {
    ...
    async init() {
        ...
        // 检测当前地图引擎
        const { MapConfig } = await import('../config/map.config.js');
     ...
     * 测试 2: 适配器接口完整性测试
     */
    async testAdapterInterface(adapter) {
     ...
     * 测试 4: GeoJSON 图层测试
     */
    async testGeoJSONLayer(adapter) {
     ...
     * 测试 5: 图层控制测试
     */
    async testLayerControl(adapter) {
    ...
    async testCoordinateSystem() {
        ...
        try {
            const { CoordinateTransformer } = await import('../utils/coordinateTransformer.js');
    ...
    async testConfiguration() {
        ...
        try {
            ...
            if (!provider || (provider !== 'arcgis' && provider !== 'tianditu')) {
                throw new Error('地图引擎配置无效');
            }

            if (provider === 'arcgis') {
                const { ArcGISConfig } = await import('../config/arcgis.config.js');
                const config = ArcGISConfig.getConfig();
                if (!config) {
                    throw new Error('ArcGIS 配置加载失败');
                }
            } else if (provider === 'tianditu') {
                const { TiandituConfig } = await import('../config/tianditu.config.js');
                const config = TiandituConfig.getConfig();
                if (!config) {
                    throw new Error('天地图配置加载失败');
                }
            }
...
// 导出便捷函数
export async function runMapEngineTests(adapter) {
