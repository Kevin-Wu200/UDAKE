...
export class SinglePointSampling {
     ...
     */
    bindEvents() {
        const addBtn = this.container.querySelector('#add-point-btn');
    ...
    async handleAddPoint() {
        const xInput = this.container.querySelector('#point-x');
        const yInput = this.container.querySelector('#point-y');
        const dataInput = this.container.querySelector('#point-data');
        ...
        try {
            ...
            if (this.onPointAdded) {
                this.onPointAdded({ x: point.x, y: point.y, value });
            }
            ...
            this.showError('point-x', error.message);
        }
    }
     ...
     * 解析坐标
     */
    async parseCoordinate(value, axis) {
        // 检测格式
    ...
    showError(inputId, message) {
        ...
        const errorSpan = this.container.querySelector(`#error-${inputId.split('-')[1]}`);
