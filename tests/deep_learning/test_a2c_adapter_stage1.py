...
def _uncertainty_map(size: int = 10, seed: int = 71) -> np.ndarray:
    ...
    base = 0.49 + 0.24 * np.sin(xx * 3.0) + 0.2 * np.cos(yy * 4.1)
...
def _build_observations(n: int = 18) -> list[dict[str, np.ndarray]]:
    ...
    agent = ActorCriticAgent(action_dim=env.h * env.w, seed=79)
    obs = env.reset()
...
def test_a2c_preprocess_predict_and_lime_shap_adapters() -> None:
    observations = _build_observations(n=16)
    agent = ActorCriticAgent(action_dim=100, seed=83)

    pre = agent.preprocess_a2c_data(observations, use_training_stats=True)
    ...
    pred = agent.predict_a2c(observations, deterministic=True)
