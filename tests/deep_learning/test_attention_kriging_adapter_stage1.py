...
def test_attention_kriging_preprocess_and_predict_standard() -> None:
    coords, values = _make_data()
    model = AttentionKrigingModel(dim=24)
    queries = coords[:12]

    pre = model.preprocess_attention_kriging_data(coords, values, query_coords=queries, batch_size=6, use_runtime_stats=True)
    ...
    assert np.asarray(pre["attention_weights"], dtype=float).shape == (12, coords.shape[0])
    ...
    assert len(pred["attention_summary"]["mean_weight"]) == 12


def test_attention_kriging_lime_and_shap_adapters() -> None:
    coords, values = _make_data(seed=31)
    model = AttentionKrigingModel(dim=24)
    queries = coords[:18]

    lime_adapter = AttentionKrigingLIMEAdapter(
        config=AttentionKrigingExplanationConfig(lime_num_samples=120, max_explain_nodes=4)
    )
    ...
    shap_adapter = AttentionKrigingSHAPAdapter(
        config=AttentionKrigingExplanationConfig(shap_nsamples=90, max_explain_nodes=4)
    )
...
def test_attention_kriging_adapter_cache_hit() -> None:
    coords, values = _make_data(seed=37)
    model = AttentionKrigingModel(dim=24)
    queries = coords[:16]
    adapter = AttentionKrigingSHAPAdapter(
        config=AttentionKrigingExplanationConfig(shap_nsamples=80, max_explain_nodes=3)
    )
