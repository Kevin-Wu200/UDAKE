...
@dataclass
class MCDropoutExplanationConfig:
...
class _BaseMCDropoutAdapter:
    def __init__(self, config: Optional[MCDropoutExplanationConfig] = None) -> None:
    ...
    def _build_context(self, model: Any, features: np.ndarray) -> dict[str, Any]:
        ...
        pre = model.preprocess_mc_dropout_data(x, use_training_stats=True)
        pred = model.predict_mc_dropout(
            x,
        ...
        if x_scaled.shape[0] != y_mean.shape[0]:
            raise ValueError("MC Dropout 预处理与预测样本数量不一致")
...
class MCDropoutLIMEAdapter(_BaseMCDropoutAdapter):
    """MC Dropout 的 LIME 解释适配器。"""
...
class MCDropoutSHAPAdapter(_BaseMCDropoutAdapter):
    """MC Dropout 的 SHAP 解释适配器。"""
