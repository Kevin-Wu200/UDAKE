...
class _BaseBNNAdapter:
    def __init__(self, config: Optional[BNNExplanationConfig] = None) -> None:
        self.config = config or BNNExplanationConfig()
    ...
    def _model_fingerprint(self, model: Any) -> str:
        ...
        for attr in ("hidden", "mean_head", "logvar_head"):
            ...
            for pname in ("weight", "bias"):
                ...
                if param is None:
                    continue
                mu = np.asarray(getattr(param, "mu", np.array([0.0])), dtype=float)
                rho = np.asarray(getattr(param, "rho", np.array([0.0])), dtype=float)
                parts.extend(
                    [
                        float(np.mean(mu)),
                        float(np.std(mu)),
    ...
    def _build_context(self, model: Any, features: np.ndarray) -> dict[str, Any]:
        ...
        pre = model.preprocess_bnn_data(x, use_training_stats=True)
        pred = model.predict(
            np.asarray(pre["processed_features"], dtype=float),
            num_samples=max(24, int(self.config.shap_nsamples)),
        )
        ...
        if x_scaled.shape[0] != y_mean.shape[0]:
            raise ValueError("BNN 预处理与预测样本数量不一致")
    ...
    def _bnn_advanced_analysis(
        ...
        *,
        ...
        weight_expl = (
            model.explain_bayesian_weights(top_k=max(4, int(top_k)))
            if hasattr(model, "explain_bayesian_weights")
            else {}
        )
        posterior = model.analyze_posterior_distributions() if hasattr(model, "analyze_posterior_distributions") else {}
        epistemic = (
            model.analyze_epistemic_uncertainty(
                features,
                num_samples=max(24, int(self.config.shap_nsamples)),
                top_k=max(4, int(top_k)),
                use_training_stats=True,
            )
            if hasattr(model, "analyze_epistemic_uncertainty")
            else {}
        )
        return {
            "bayesian_weight_explanation": weight_expl,
            "posterior_distribution_analysis": posterior,
            "epistemic_uncertainty_analysis": epistemic,
        }
...
class BNNLIMEAdapter(_BaseBNNAdapter):
    """BNN 的 LIME 解释适配器。"""
    ...
    def explain(
        ...
        *,
        ...
        for idx in node_indices:
            ...
            if lime_explainer is not None:
                try:
                    ...
                    local_pred = _safe_float(getattr(exp, "predicted_value", [0.0])[0], float(predict_fn(instance.reshape(1, -1))[0]))
                except Exception:
        ...
        advanced = self._bnn_advanced_analysis(model=model, features=x, top_k=int(top_k))
        ...
        payload = {
            ...
            "bayesian_weight_explanation": advanced["bayesian_weight_explanation"],
            "posterior_distribution_analysis": advanced["posterior_distribution_analysis"],
            "epistemic_uncertainty_analysis": advanced["epistemic_uncertainty_analysis"],
...
class BNNSHAPAdapter(_BaseBNNAdapter):
    """BNN 的 SHAP 解释适配器。"""
