...
import numpy as np
from app.dl_services.shap_explainer import SpatiotemporalSHAPExplainer
...
def test_shap_explainer_generates_visualization_payload() -> None:
    ...
    assert len(result["visualization"]["feature_ranking"]) >= 1
    assert len(result["batch_explanations"]) >= 1


def test_shap_explainer_result_cache() -> None:
    coords, series, pred_mean = _build_data()
    explainer = SpatiotemporalSHAPExplainer()
    ...
    assert second["summary"]["method"] == "shap"
