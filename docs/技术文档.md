...
##### 1. 地图引擎 API
**问题**: 第三方地图库（GeoScene、高德地图）类型定义不完整
**解决方案**:
...
##### 必要的 `any` 使用
1. **第三方库集成**: GeoScene、高德地图、ECharts 等库的类型定义不完整
...
**涉及文件**：
- `map/core/GeoSceneEngine.ts` (4 个)
- `主程序.ts` (6 个)
...
## 克里金参数可视化功能（2026-04 补充）

### 组件范围

- `ParameterAdjustmentPanel`：参数关系图切换、状态持久化、参数联动
- `ParameterRelationshipChart`：多维关系图渲染、约束区域高亮、历史轨迹
- `ParameterImpactPreview`：参数影响预览、指标估算、预览导出
- `VariogramChart`：实时变异函数拟合与质量建议
...
2. `range-spatial`
- X 轴：`range`（0-100）
- Y 轴：`空间范围`（由采样点边界对角线动态计算）
- 推荐区间：`range` 取 `空间范围` 的 15%-40%

3. `grid-performance`
- X 轴：`grid-resolution`（50-500）
- Y 轴：`预估耗时(s)`（0-30）
- 推荐约束：预估耗时 < 10s

### 持久化与同步策略

- 关系图类型通过 `localStorage['krigingRelationshipType']` 持久化。
- 面板初始化优先恢复上次关系图类型。
- 参数变化时会驱动关系图、影响预览、变异函数拟合的同步刷新。

### 参数配置最佳实践

1. 小样本数据（< 50 点）：优先使用 `balanced` 模板，`grid-resolution` 建议 80-140。
2. 中等样本（50-300 点）：保持 `range` 在空间范围 15%-40%，避免过平滑或过拟合。
3. 大样本（> 300 点）：先控制 `grid-resolution`，确保预估耗时 < 10s，再微调 `nugget/sill`。
4. 调参顺序建议：先 `range`，再 `sill`，最后 `nugget` 与 `nlags`。
...
重点覆盖点：初始化、更新流程、约束验证、交互回写、缓存管理、导出失败兜底。
...
- `apps/frontend/js/components/ParameterRelationshipChart.ts`
  - 新增鼠标悬浮与触摸提示，显示 X/Y 精确值与状态。
  - 支持历史轨迹点的数值提示。

- `apps/frontend/js/components/ParameterAdjustmentPanel.ts`
  - 在约束标签旁新增“恢复到推荐区”按钮。
  - 根据当前关系类型计算推荐区中心点并带动画回写参数。

- `apps/frontend/js/components/ParameterImpactPreview.ts`
  - 预览图生成失败降级为无图模式。
  - 导出操作增加异常保护，避免 UI 中断。
...
- 覆盖文件：
  - `tests/ParameterImpactPreview.test.ts`
  - `tests/VariogramChart.test.ts`
  - `tests/ParameterAdjustmentPanel.test.ts`
  - `tests/ParameterRelationshipChart.test.ts`
- 重点场景：
  - 失败分支、空数据分支、边界分支、交互分支、导出异常分支。

---

## 企业管理员权限系统优化（2026-04-15）

### 前端
- `ProductKeysView.vue`
  - 增加过期时间列、过期红色标识、到期倒计时。
  - 增加企业管理员配额提示与创建禁用逻辑。
- 新增页面：
  - `CompanyAdminProfile.vue`
  - `SMTPSettings.vue`
  - `EmailLogs.vue`
- `KeyGenerateDialog.vue`
  - 支持按管理员类型限制密钥类型。
  - 支持剩余配额约束和数量校验。

### 后端
- 新增调度模块：
  - `services/backend/app/scheduler/tasks.py`
  - `services/backend/app/scheduler/key_expiry_scheduler.py`
- 新增调度管理 API：
  - `services/backend/app/api/scheduler_api.py`
- `main.py` 生命周期集成调度器启动与停止。
- `config.py` 新增调度与提醒重试配置项。

### 调度任务能力
- 自动任务：过期检查、到期提醒。
- 手动任务：可通过 API 手动触发。
- 管理能力：查看历史、暂停/恢复、修改执行时间。
- 提醒任务具备失败重试与执行统计输出。
