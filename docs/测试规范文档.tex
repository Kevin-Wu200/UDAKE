...
\begin{itemize}
    \item \texttt{services/backend/tests/test\_model\_fusion.py} - 模型融合系统测试
    \item \texttt{services/backend/tests/test\_route\_planning.py} - 路径规划系统测试
    \item \texttt{services/backend/tests/test\_anomaly\_detector.py} - 异常检测器测试
    \item \texttt{services/backend/tests/test\_decision\_threshold\_analyzer.py} - 决策阈值分析器测试
    \item \texttt{services/backend/tests/test\_error\_predictor.py} - 误差预测器测试
    \item \texttt{services/backend/tests/test\_model\_evaluator.py} - 模型评估器测试
    \item \texttt{services/backend/tests/test\_risk\_index\_calculator.py} - 风险指数计算器测试
    \item \texttt{services/backend/tests/test\_sampling\_algorithms.py} - 采样算法测试
    \item \texttt{services/backend/tests/test\_spatial\_risk\_reporter.py} - 空间风险报告器测试
    \item \texttt{services/backend/tests/test\_uncertainty\_classifier.py} - 不确定性分类器测试
    \item \texttt{services/backend/tests/test\_websocket.py} - WebSocket服务测试
    \item \texttt{services/backend/test\_backend.py} - 后端集成测试
\end{itemize}
...
# 运行特定测试文件
pytest services/backend/tests/test_model_fusion.py

# 运行特定测试函数
pytest services/backend/tests/test_model_fusion.py::TestWeightCalculator::test_calculate_weights_rmse_based
...
# 运行特定异步测试
pytest services/backend/tests/test_websocket.py::test_websocket_connection
\end{lstlisting}
...
\textbf{文件名} & \textbf{测试内容} & \textbf{测试用例数} \\
\hline
\texttt{services/backend/tests/test\_model\_fusion.py} & 模型融合系统 & 30 \\
\texttt{services/backend/tests/test\_route\_planning.py} & 路径规划系统 & 25 \\
\texttt{services/backend/tests/test\_anomaly\_detector.py} & 异常检测器 & 15 \\
\texttt{services/backend/tests/test\_decision\_threshold\_analyzer.py} & 决策阈值分析器 & 12 \\
\texttt{services/backend/tests/test\_error\_predictor.py} & 误差预测器 & 15 \\
\texttt{services/backend/tests/test\_model\_evaluator.py} & 模型评估器 & 20 \\
\texttt{services/backend/tests/test\_risk\_index\_calculator.py} & 风险指数计算器 & 12 \\
\texttt{services/backend/tests/test\_sampling\_algorithms.py} & 采样算法 & 18 \\
\texttt{services/backend/tests/test\_spatial\_risk\_reporter.py} & 空间风险报告器 & 10 \\
\texttt{services/backend/tests/test\_uncertainty\_classifier.py} & 不确定性分类器 & 15 \\
\texttt{services/backend/tests/test\_websocket.py} & WebSocket服务 & 20 \\
\texttt{services/backend/test\_backend.py} & 后端集成测试 & 25 \\
\hline
\end{tabular}
\end{table}
