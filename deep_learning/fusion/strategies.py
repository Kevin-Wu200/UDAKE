...
class FusionStrategies:
    ...
    def simple_average(self, models: list[ModelPrediction], enable_uncertainty: bool = True) -> tuple[list[float], list[float] | None]:
    ...
    def median(self, models: list[ModelPrediction], enable_uncertainty: bool = True) -> tuple[list[float], list[float] | None]:
        matrix = ensure_prediction_matrix(models)
    ...
    def max_min(self, models: list[ModelPrediction], enable_uncertainty: bool = True) -> tuple[list[float], list[float] | None]:
        matrix = ensure_prediction_matrix(models)
    ...
    def stacking(
        ...
        if true_values is None or len(true_values) != matrix.shape[1] or matrix.shape[1] < 3:
            fused, var = self.weighted_average(models, weights, enable_uncertainty=enable_uncertainty)
    ...
    def variance_weighted(
        ...
        if not all(m.variances is not None for m in models):
            return self.weighted_average(models, fallback_weights, enable_uncertainty=enable_uncertainty)
